**🛍️ Customer Shopping Behavior Analysis**

**📌 Overview**

This project analyzes customer shopping behavior using data analytics techniques to uncover meaningful insights that support business decision-making. It covers the complete analytics workflow — from data loading and cleaning to visualization and reporting.



**📂 Dataset**
customer_shopping_behavior.csv

Contains transactional and customer-related data used for analysis.


Business Problem Statement.pdf

Defines the objectives and goals of the analysis.




**🛠️ Tools & Technologies**

Python (Pandas) – Data loading & preprocessing

Jupyter Notebook – Analysis & EDA

MS SQL Server – Querying & data extraction

Power BI – Interactive dashboard

Gamma AI – Presentation creation

Excel / CSV – Data source




**🔄 Project Workflow**
1. Data Loading & Cleaning
Imported dataset using Pandas
Handled missing values and duplicates
Standardized and formatted data

3. Exploratory Data Analysis (EDA)
Analyzed customer demographics
Studied purchasing patterns
Identified trends and correlations

5. SQL Analysis
Wrote queries in MS SQL Server
Extracted key business insights
Aggregated and filtered data

7. Dashboard Development
Built an interactive Power BI dashboard
Visualized KPIs and customer insights
Enabled easy business interpretation

9. Reporting & Presentation
Created detailed analysis report
Designed presentation using Gamma AI




**📊 Dashboard**
The Power BI dashboard provides a comprehensive view of customer shopping behavior with interactive filters and key performance metrics:


🔑 Key Metrics (KPIs)

3.9K → Total Number of Customers

3.75 → Average Review Rating

$59.76 → Average Purchase Amount


📌 Customer Insights

Subscription distribution (73% non-subscribed, 27% subscribed)

Gender-based and category-based filtering

Customer segmentation by age group


📈 Sales & Revenue Analysis

Revenue breakdown by product categories (Clothing, Accessories, Footwear, Outerwear)

Sales distribution across categories

Revenue and sales trends by age group


🎛️ Interactive Features

Filters for:

Subscription Status

Gender

Product Category

Shipping Type

Dynamic visuals for deeper analysis



📁 File: customer behavior dashboard.pbix




**📈 Results & Insights**
Identified high-value customer segments

Discovered peak shopping periods

Highlighted top-performing categories

Provided actionable insights for business growth




**📁 Project Files**
Customer-Shopping-Behavior-Analysis/

│

├── customer_shopping_behavior.csv

├── Business Problem Statement.pdf

├── customer_shopping_behavior.ipynb

├── CutomerShoppingBehavior.sql

├── customer behavior dashboard.pbix

├── Customer Shopping Behavior Analysis Report.pdf

├── Customer-Shopping-Behavior-Analysis.pdf

└── README.md




**👤 Author**
Shanu Shahid

MBA Student | Aspiring Data Analyst

Interested in Data Analytics, Business Insights & Digital Strategy




**⭐ Key Highlights**

End-to-end data analytics project

Combines Python, SQL, and BI tools

Business-focused insights and reporting

Recruiter-friendly project structure
